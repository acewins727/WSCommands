		...
		X = 1;
		{
			try {
				...
				if (commands.get("DedicatedMode").getAsBoolean()) {
					if (!(entity instanceof ServerPlayer _plr4 && _plr4.level() instanceof ServerLevel && _plr4.getAdvancements().getOrStartProgress(_plr4.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"))).isDone())) {
						if (!commands.get("Repeat").getAsBoolean()) {
							if (entity instanceof ServerPlayer _player) {
								AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"));
								if (_adv != null) {
					...
					if (!(entity instanceof ServerPlayer _plr11 && _plr11.level() instanceof ServerLevel && _plr11.getAdvancements().getOrStartProgress(_plr11.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"))).isDone())) {
						if (!commands.get("Repeat").getAsBoolean()) {
							if (entity instanceof ServerPlayer _player) {
								AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"));
								if (_adv != null) {
