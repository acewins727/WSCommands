		...
		X = 1;
		{
			try {
				...
				if (commands.get("DedicatedMode").getAsBoolean()) {
					if (!(entity instanceof ServerPlayer _plr3 && _plr3.level() instanceof ServerLevel && _plr3.getAdvancements().getOrStartProgress(_plr3.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"))).isDone())) {
					...
					if (world.players().size() == 1) {
						if (!(entity instanceof ServerPlayer _plr10 && _plr10.level() instanceof ServerLevel
								&& _plr10.getAdvancements().getOrStartProgress(_plr10.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"))).isDone())) {
							if (!commands.get("Repeat").getAsBoolean()) {
								if (entity instanceof ServerPlayer _player) {
									AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("wscommands:new_world"));
									if (_adv != null) {
